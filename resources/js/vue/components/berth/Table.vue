<template>
	<div class="mt-1">
		<form>
			<button type="button" class="btn btn-danger" @click.prevent="deleteSelectedItems">ausgewähle löschen
			</button>
		</form>
		...
		<table v-if="data" class="table table-striped table-sm mt-1">
			...
			<tr v-for="item in data" :key="item.id">
				...
				<td><a href="#" class="text-decoration-none" @click.prevent="selectItem(item)">{{ item.number }}</a>
				</td>
				...
				<td>
					<Toggle :item="item" field="enabled"/>
				</td>
