...
@section('main')
	<div>
		<x-btn-back route="{{ route('customer.boatDates.index', ['saison' => $modus ?? null]) }}"/>
		<div class="mt-2">
			<span class="text-xl text-blue-900">Boot: {{ $boatDate->boat->name }} ({{ __($boatDate->modus) }})</span>
		</div>
		<x-form name="frm" method="post" :action="route('customer.boatDates.update', $boatDate)" class="w-half mt-3">
			@method('put')
			@bind($craneDate)
