...
@section('main')
	<div>
		...
		<x-form name="frm" method="post" :action="route('admin.apartmentModels.update', $apartmentModel)"
				class="w-half mt-3">
			@method('put')
			@bind($apartmentModel)
	...
	<script type="module">
		$(document).ready(() => {
			const route = "{{ route('admin.upload.image') }}",
				model_type = "{{ addslashes(get_class($apartmentModel)) }}",
				model_id = {{ $apartmentModel->id }},
				files = {!! $files !!};
