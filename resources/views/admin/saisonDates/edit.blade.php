...
@section('main')
    <div class="p-6">
        <x-nav-link :href="route('admin.saisonDates.index')" icon="fas fa-backward" class="btn">zurück</x-nav-link>
        <x-form method="post" :action="route('admin.saisonDates.update', $saisonDate)" class="w-full lg:w-1/2">
            @method('put')
            @bind($saisonDate)
