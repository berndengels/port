		...
		<table class="table m-10 show-table">
			...
			<tr>
				<td colspan="2">
					<div class="w-100 border-secondary mt-3">
						<x-form method="post" :action="route('admin.serviceRequests.done', $serviceRequest)"
								class="row">
							@method('post')
							...
							<div class="col-auto">
								<x-form-submit floating type="submit" class="btn-sm btn-primary d-inline-block">
									Speichern
								</x-form-submit>
							</div>
						</x-form>
			...
			@foreach($serviceRequest->services as $service)
				...
				<tr>
					...
					<td>{{ ($service->getServicePrice($serviceRequest->boat)) }} € ({{ $service->price }}
						€ {{ $service->priceType->name }})
					</td>
				</tr>
						...
						<td>
							<ul class="list-disc ml-3">
								@foreach($service->materials as $material)
									<li>{{ $material->name }}</li>
									<li>Preis: {{ $material->price_per_unit }} €
										pro {{ $material->priceType->unit }}</li>
									<li>Ergiebigkeit: {{ $material->fertility }} {{ $material->fertility_unit }}
										pro {{ $material->fertility_per }}</li>
									<li>benötigte
										Materialmenge: {{ round($material->getQuantity($serviceRequest->boat),1) }} {{ $material->fertility_per }}</li>
									<li>berechneter
										Materialpreis: {{ round($material->getMaterialPrice($serviceRequest->boat),1) }}
										€
									</li>
								@endforeach
							</ul>
