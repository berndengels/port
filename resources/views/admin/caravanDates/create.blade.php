...
@section('main')
	<div>
		...
		<x-form name="frm" method="post" action="{{ route('admin.caravanDates.store') }}" class="w-half mt-3">
			...
			<x-form-input id="carnumber" name="carnumber" type="text" label="Kennzeichen" placeholder="Auto-Kennzeichen"
						  autocomplete="off" required/>
			<ul class="d-none autocomplete"></ul>
			<x-form-select id="country_id" name="country_id" label="Herkunftsland" :options="$countries"
						   default="{{ config('port.main.default.country_id') }}"/>
