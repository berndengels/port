...
@section('main')
	<div>
		<x-btn-back route="{{ route('admin.apartmentRentals.index', ['saison' => $modus ?? null]) }}"/>
		<div class="flex-row">
			<div class="flex-column">
				<x-form name="frm" method="post" :action="route('admin.apartmentRentals.update', $houseboatDate)"
						class="w-half mt-3">
					@method('put')
					<div class="my-1">
					...
					<input type="hidden" class="calc" id="houseboat_id" name="houseboat_id"
						   value="{{ $houseboatDate->houseboat_id }}"/>
					@bind($houseboatDate)
...
@push('inline-scripts')
	<script>
		$(document).ready(() => {
			...
			const calcUrl = "{{ route('admin.apartmentRentals.price.calculate') }}";
			Prices.apartmentRentals.calculate(document.frm, calcUrl);
		});
	</script>
@endpush
