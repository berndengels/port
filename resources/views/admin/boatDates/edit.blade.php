...
@section('main')
    <div class="p-6">
        ...
        <x-form name="frm" method="post" :action="route('admin.boatDates.update', $boatDate)" class="w-full lg:w-1/2">
            ...
            <x-form-input class="calc" name="from" type="date" label="Von" required :default="$boatDate->from->format('Y-m-d')" />
            <x-form-input class="calc" name="until" type="date" label="Bis" required :default="$boatDate->validUntil" />
