...
@section('main')
	<div>
		...
		<x-form class="w-half mt-3" method="post"
				:action="route('admin.config.saisonRentDates.update', $saisonRentDate)">
			@method('put')
			@bind($saisonRentDate)
			...
			<x-form-input type="date" id="from" name="from" :bind="false" :default="$saisonRentDate->validFrom"
						  label="Von" required/>
			<x-form-input type="date" id="until" name="until" label="Bis" :bind="false"
						  :default="$saisonRentDate->validUntil" required/>
			@endbind
			<div class="mt-2">
