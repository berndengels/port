...
@section('main')
    <div>
        <div class="index-header mt-3">
            <div>
                <x-nav-link
                        href="{{ route('admin.config.saisonRentDates.create') }}"
                        class="btn"
        ...
        <table class="table w-full mt-3">
            ...
            @foreach($data as $item)
                <tr>
                    ...
                    <td>{{ $item->strUntil }}</td>
                    <td>
                        <x-nav-link href="{{ route('admin.config.saisonRentDates.edit', $item) }}" icon="fas fa-edit" class="btn" title="Bearbeiten">
                            <span class="hidden md:visible">Edit</span>
                        </x-nav-link>
                    </td>
                    <td>
                        <x-form action="{{ route('admin.config.saisonRentDates.destroy', $item) }}"
                                class="m-0 p-0">
                            @method('delete')
