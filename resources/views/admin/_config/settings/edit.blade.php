		...
		<div class="row mt-3">
			<div class="col-sm-12 col-md-4">
				<x-form method="post" :action="route('admin.config.settings.update', $setting)" class="w-full">
					...
					<x-form-input id="bic" name="bic" label="BIC"
								  pattern="^([a-zA-Z]){6}([0-9a-zA-Z]){2}([0-9a-zA-Z]{3})?$"
								  title="Ein BIC hat 8 oder 11 Stellen" placeholder="BIC"/>
					<x-form-input id="iban" name="iban" label="IBAN" placeholder="IBAN"
								  pattern="^DE\d{2}[ ]\d{4}[ ]\d{4}[ ]\d{4}[ ]\d{4}[ ]\d{2}|DE\d{20}$"
								  title="Eine deutsche IBAN hat 22 Stellen und beginnt mit DE"/>
					<x-form-input type="number" step="0.1" min="0" id="tax" name="tax" label="MWSt"/>
					<x-form-checkbox id="use_tax" name="use_tax" label="MWSt im Preis ansetzen"/>
...
@push('inline-scripts')
	<script>
		$(document).ready(() => {
			...
			if (lat && lng) {
				const geo = new Geo(lat.value, lng.value, zoom);
				geo.initMap('map');
