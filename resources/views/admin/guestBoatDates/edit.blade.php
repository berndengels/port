...
@section('main')
	<div>
		...
		<x-form name="frm" method="post" :action="route('admin.guestBoatDates.update', $guestBoatDate)"
				class="w-half mt-3">
			@method('put')
			...
			<x-form-input id="length" class="calc" name="length" type="number" step="1" min="1" label="Boots Länge"
						  placeholder="Boots Länge" readonly required/>
			...
			<x-form-input name="from" class="calc" type="date" label="Von" required :bind="false"
						  default="{{ $guestBoatDate->validFrom }}"/>
			<x-form-input name="until" class="calc" type="date" label="Bis" required :bind="false"
						  default="{{ $guestBoatDate->validUntil }}"/>
			<div class="mt-3">
