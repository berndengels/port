...
@section('main')
    <div class="p-6">
        <x-nav-link :href="route('admin.boatPrices.index')" icon="fas fa-backward" class="btn">zurück</x-nav-link>
        <x-form method="post" :action="route('admin.boatPrices.update', $boatPrice)" class="w-full lg:w-1/2">
            @method('put')
            @bind($boatPrice)
