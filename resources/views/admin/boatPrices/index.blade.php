...
@section('main')
    <div>
        <div class="index-header mt-3">
            <div>
                <x-nav-link
                        href="{{ route('admin.boatPrices.create') }}"
                        class="btn"
        ...
        <table class="table w-full mt-3">
            ...
            @foreach($data as $item)
                <tr>
                    ...
                    <td>
                        <x-nav-link href="{{ route('admin.boatPrices.edit', $item) }}" icon="fas fa-edit" class="btn" title="Bearbeiten">
                            <span class="hidden md:visible">Edit</span>
                        </x-nav-link>
                    </td>
                    <td>
                        <x-form action="{{ route('admin.boatPrices.destroy', $item) }}"
                                class="m-0 p-0">
                            @method('delete')
