        ...
        <div class="flex-row">
            <div class="flex-column">
                <x-form name="frm" action="{{ route('admin.'.$routeName.'.update', $rental) }}" class="w-half mt-3">
                    ...
                    <input type="hidden" class="calc" id="rentable_id" name="rentable_id" value="{{ $rental->rentable_id }}" />
					<input type="hidden" class="calc" id="rentable_type" name="rentable_type" value="{{ $rental->rentable_type }}" />
					@bind($rental)
					...
					@if($priceComponents->count() > 0)
						@foreach($priceComponents as $pc)
							...
							<div class="mt-3">
								<x-form-checkbox class="calc" id="{{ $pc->key }}" name="{{ $pc->key }}" label="{{ $pc->name }}" default="{{ $has }}"/>
								@if('kilowatt' === $pc->key)
									<x-form-input class="calc" id="kilowatt_value" name="kilowatt_value" type="number" min="0" step="0.1" value="0" label="Stromverbrauch in Kw"/>
								@endif
							</div>
						@endforeach
					@endif
