...
@section('main')
    <div class="container">
        <div class="w-100">
            ...
            <br>
            <br>
            <x-recipient-header class="mt-5" :recipient="$rental->customer" />
            <h5 class="w-100">Rechnung Miete für das {{ __(class_basename($rental->rentable)) }} {{ $rental->rentable->name }} vom {{ $rental->from->format('d.m.Y') }} bis {{ $rental->until->format('d.m.Y') }}</h5>
            <!--div class="row mt-3">
                <div class="col-3">Objekt</div>
                <div class="col">{{ $rental->rentable->name }}</div>
            </div-->
            <div class="row">
                <div class="col-3">Mieter</div>
                <div class="col">{{ $rental->customer->name }}</div>
            </div>
            <div class="row">
