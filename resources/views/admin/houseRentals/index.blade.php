    ...
    <div>
        <div class="index-header mt-3 p-0">
            <div class="float-start">
                <x-btn-create route="{{ route('admin.houseRentals.create', ['modus' => $saison ?? null]) }}" />
            </div>
            @if($data && $data->count() > 0 && $year || $month)
                <div class="float-end">
                    <x-frm-excel
                        :action="route('admin.houseRentals.sendExcel')"
        ...
        <x-form class="inline-form ml-5" method="get" id="frmFilter" name="frmFilter"
                action="{{ route('admin.houseRentals.index') }}"
        >
    ...
    @if($data && $data->count() > 0)
        ...
        <x-table :items="$data" :fields="['Hausboot','Von','Bis','Gast:md','Fon','Preis:md','Bezahlt']" hasActions>
            @foreach($data as $item)
                <tr>
                    @bindData($item)
                    <x-td field="rentable.name" link="{{ route('admin.houseRentals.show', $item) }}" />
                    <x-td field="from" />
                    <x-td field="until" />
                    ...
                    <x-td field="price:md" />
                    <x-td field="is_paid" />
                    <x-action routePrefix="admin.houseRentals" edit delete />
                    @endBindData
                </tr>
            @endforeach
        </x-table>
...
@push('inline-scripts')
    <script>
        $(document).ready(() => {
	        ...
	        Edit.toggle("/admin/houseRentals/toggle","is_paid");
