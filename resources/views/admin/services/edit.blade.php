...
@section('main')
	<div>
		...
		<x-form method="post" :action="route('admin.services.update', $service)" class="w-half mt-3">
			@method('put')
			@bind($service)
			<x-form-input name="name" label="Beschreibung" required/>
			<x-form-select name="materials[]" label="Material" :options="$materials" class="flexy" size="10"
						   many-relation multiple/>
			<x-form-select name="price_type_id" label="Arbeits-Preis-Type" :options="$priceTypes" required/>
			<x-form-input type="number" min="1" step="1" name="quantity" label="Anzahl"/>
