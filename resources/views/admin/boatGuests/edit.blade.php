...
@section('main')
    <div class="p-6">
        <x-nav-link :href="route('admin.boatGuests.index')" icon="fas fa-backward" class="btn">zurück</x-nav-link>
        <x-form name="frm" method="post" :action="route('admin.boatGuests.update', $boatGuest)" class="w-full lg:w-1/2">
            @method('put')
            @bind($boatGuest)
