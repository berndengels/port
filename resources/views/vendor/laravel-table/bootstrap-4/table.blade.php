			...
			<table class="table table-borderless">
									...
									<div wire:ignore @class(['px-xl-3' => $headActionArray, 'pl-xl-3' => ! $headActionArray, 'py-1'])>
										<div class="input-group">
											...
											<select wire:change="changeNumberOfRowsPerPage($event.target.value)"
													class="custom-select" {!! (new ComponentAttributeBag())->merge([
												...
												<option wire:key="rows-number-per-page-option-placeholder" value=""
														disabled>{{ __('Number of rows per page') }}</option>
				...
				<tr class="table-light border-top border-bottom">
					...
					@if($tableBulkActionsArray)
						<th wire:key="bulk-actions" class="align-middle" scope="col">
							<div class="d-flex align-items-center">
								...
								<input wire:model="selectAll" class="mr-1" type="checkbox"
					...
					@foreach($columns as $column)
						<th wire:key="column-{{ Str::of($column->getAttribute())->snake('-')->slug() }}"
						...
						@if($tableBulkActionsArray)
							<td class="align-middle">
								<input wire:model="selectedModelKeys" type="checkbox" value="{{ $model->getKey() }}"
									   aria-label="Check line {{ $model->getKey() }}">
							</td>
						@endif
						{{-- Row columns values --}}
						@foreach($columns as $column)
							@if($loop->first)
								<th wire:key="cell-{{ Str::of($column->getAttribute())->snake('-')->slug() }}-{{ $model->getKey() }}"
									{!! $orderColumn ? ' wire:sortable.handle style="cursor: move;"' : null !!} class="align-middle"
									scope="row">
